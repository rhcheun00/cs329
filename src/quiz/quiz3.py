...
def create_np_dict(data: List[List[Tuple[str, str]]]) -> Dict[str, List[Tuple[str, float]]]:
    """
    :param data: a list of tuple lists where each inner list represents a sentence and every tuple is a (word, pos) pair.
    :return: a dictionary where the key is the previous pos and the value is the list of possible POS tags with probabilities in descending order.
    """
    model = dict()
    for sentence in data:
        for i, (_, curr_pos) in enumerate(sentence):
...
def create_tpp_dict(data: List[List[Tuple[str, str]]]) -> Dict[str, List[Tuple[str, float]]]:
    """
    :param data: a list of tuple lists where each inner list represents a sentence and every tuple is a (word, pos) pair.
    :return: a dictionary where the key is the previous 2 POS tag and the value is the list of possible POS tags with probabilities in descending order.
    """
    model = dict()
    for sentence in data:
        for i, (_, curr_pos) in enumerate(sentence):
            prev_pos = sentence[i-1][1] if i > 1 else DUMMY
            pprev_pos = sentence[i-2][1] if i > 1 else DUMMY
            tprev_pos = prev_pos + pprev_pos
            model.setdefault(tprev_pos, Counter()).update([curr_pos])
    return to_probs(model)
...
def train(trn_data: List[List[Tuple[str, str]]], dev_data: List[List[Tuple[str, str]]]) -> Tuple:
    ...
    tpp_dict = create_tpp_dict(trn_data)
    ...
    for pp_weight in grid:
        for pw_weight in grid:
            for nw_weight in grid:
                for np_weight in grid:
                    for tpp_weight in grid:
                        args = (cw_dict, pp_dict, pw_dict, nw_dict, np_dict, tpp_dict, cw_weight, pp_weight, pw_weight, nw_weight, np_weight, tpp_weight)
                        acc = evaluate(dev_data, *args)
                        print('{:5.2f}% - cw: {:3.1f}, pp: {:3.1f}, pw: {:3.1f}, nw: {:3.1f}, np: {:3.1f}, tpp: {:3.1f}'.format(acc, cw_weight, pp_weight, pw_weight, nw_weight, np_weight, tpp_weight))
...
def predict(tokens: List[str], *args) -> List[Tuple[str, float]]:
    cw_dict, pp_dict, pw_dict, nw_dict, np_dict, tpp_dict, cw_weight, pp_weight, pw_weight, nw_weight, np_weight, tpp_weight = args
    output = []
    ...
    for i in range(len(tokens)):
        ...
        pprev_pos = output[i-2][0] if i > 1 else DUMMY
        tpprev_pos = output[i-1][0] if i > 1 else DUMMY
        tprev_pos = pprev_pos + tpprev_pos
        ...
        for pos, prob in tpp_dict.get(tprev_pos, list()):
            scores[pos] = scores.get(pos, 0) + prob * tpp_weight
